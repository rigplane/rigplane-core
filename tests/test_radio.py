...
class TestCivPacingIsSendToSend:
    ...
    async def test_pump_smoke_without_pacing_clock(
        ...
        """The real pump hands one queued response to one waiter, no clock."""
        radio._civ_min_interval = 0.0
    ...
    @pytest.mark.asyncio
    async def test_pump_smoke_with_pacing_clock_gap_zero(
        self,
        radio: IcomRadio,
        mock_transport: MockTransport,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """Same handover with the clock installed but no gap to sleep."""
        clock = _install_pacing_clock(monkeypatch, gap=0.0)
        radio._civ_min_interval = 0.0
        radio._civ_ack_sink_grace = 0.0
