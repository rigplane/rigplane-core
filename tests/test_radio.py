...
class TestCivPacingIsSendToSend:
    ...
    async def test_long_reply_sends_next_immediately(
        ...
        try:
            ...
            try:
                ...
                assert await clock.wait_for_flag(second_gated)
                mock_transport.queue_response(_mode_response(Mode.USB))
    ...
    @pytest.mark.asyncio
    async def test_stale_blocking_send_still_paces_the_next(
        self,
        radio: IcomRadio,
        mock_transport: MockTransport,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """A blocking send refused after the packet left still paces next.

        Same window as the fire-and-forget variant, on the blocking path:
        ``check_current()`` runs between ``send_tracked`` and the
        ``_last_civ_send_monotonic`` stamp.  The attempt here answers a
        data query, is current while the packet leaves, and goes stale
        right after, so the refusal lands exactly in that window.
        """
        gap = 0.025
        clock = _install_pacing_clock(monkeypatch, gap=gap)
        radio._civ_min_interval = gap
        ...
        try:
            ...
            try:
                with pytest.raises(ConnectionError, match="stale"):
                    await radio._execute_civ_raw(cmd, is_current=lambda: current[0])
                assert len(mock_transport.sent_packets) == 1
                mock_transport.queue_response_on_send(2, _freq_response(14_074_000))
                frame = await radio._execute_civ_raw(cmd)
                assert frame is not None
                assert frame.command == 0x03
            finally:
    ...
    @pytest.mark.asyncio
    async def test_cancelled_frame_now_send_still_paces_the_next(
        self,
        radio: IcomRadio,
        mock_transport: MockTransport,
        monkeypatch: pytest.MonkeyPatch,
    ...
    async def test_blocking_sends_stay_one_outstanding(
        ...
        async def counting_send(data: bytes) -> None:
            await original_send(data)
            if len(mock_transport.sent_packets) == 1:
                first_sent.set()
            else:
        ...
        monkeypatch.setattr(mock_transport, "send_tracked", counting_send)
        try:
            cmd = build_civ_frame(IC_7610_ADDR, CONTROLLER_ADDR, 0x03)
            ...
            assert first is not None
            await asyncio.wait_for(second_sent.wait(), timeout=2.0)
            mock_transport.queue_response(_freq_response(14_075_000))
