...
Non-shipping test double. This module lives in the test tree and is never
packaged into the rigplane wheel; nothing under ``src/`` may import it. It
was moved out of ``src/rigplane/backends/icom7610/drivers/`` so that
TX-safety bypass sweeps over the shipping package (the MOR-999 managed-TX
audits' ``grep -rn "\\.set_ptt(" src/``) stay free of test-double false
positives — the ``set_ptt`` calls below are part of the simulated poller
dispatch, not a production TX path.
"""
...
if TYPE_CHECKING:
    from rigplane._poller_types import CommandQueue, CommandQueueEntry
