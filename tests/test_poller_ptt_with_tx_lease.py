...
# ── Per-lease intent: PTT OFF with a pushing "web" lease held ────────────────


async def test_usb_ptt_off_with_a_pushing_web_lease_keeps_tx(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """PTT OFF with a pushing "web" lease held and zero RX: TX stays live.

    Exclusive USB, keyed through the poller with no RX subscriber. The web
    lease's push records TX intent ON the lease, so releasing the "ptt"
    lease keeps tx-only demand desired as TX_ONLY — before the per-lease
    intent this idled the leg with the lease still held (measured:
    ``tx_live_after_off=False demand=1``).
    """
    radio, backend, poller, writes = _usb_stack(monkeypatch, exclusive=True)
    session = radio.audio_session
    web_lease = await session.acquire_tx("web")
    try:
        await poller._execute(PttOn(), command_id="k1", session_id="ws-1")
        assert _KEYED in writes

        await web_lease.push(_LEASE_FRAME)  # intent recorded on the lease
        assert _last_tx_frame(backend) == _LEASE_FRAME

        await poller._execute(PttOff(), command_id="k2", session_id="ws-1")
        assert _UNKEYED in writes
        assert poller._ptt_tx_lease is None  # noqa: SLF001
        # The web lease keeps the TX leg live past the unkey.
        assert radio._audio_driver.tx_running
        assert session.state is AudioSessionState.TX_ONLY
        assert session.tx_demand == 1
        await web_lease.push(_LEASE_FRAME)
...
# ── The pre-key tx_leg_live() guard (verifier round-2 advisory 4) ─────────────


class _UnobservableTxLanRadio(_PttLanRadio):
    """A transport whose ``start_tx`` returns WITHOUT arming: the TX leg is
    never observably live, so the session arm "succeeds" yet nothing can
    carry modulation. No shipped transport behaves this way (LAN, Yaesu and
    serial Icom all expose observable TX state) — the guard is defence in
    depth for one that does.
    """

    async def start_tx(self) -> None:
        self.calls.append("start_tx")  # returns without arming


async def test_unobservable_tx_leg_after_the_arm_refuses_the_key() -> None:
    """Pin the pre-key ``session.tx_leg_live()`` guard: deleting it keys the
    rig with a dead modulation path (the MOR-1178 unmodulated carrier).

    The refusal leaves no trace: no CAT key write, and ONLY the "ptt" lease
    is released — a foreign lease's demand is untouched.
    """
    radio = _UnobservableTxLanRadio()
    session = radio.audio_session
    poller = RadioPoller(radio, CommandQueue())
    foreign = await session.acquire_tx("bridge")  # bare: held, deferred
    try:
        with pytest.raises(CommandError, match="TX audio failed to arm"):
            await poller._execute(PttOn(), command_id="g1", session_id="ws-1")

        assert radio.ptt_writes == []  # no CAT key write reached the rig
        assert poller._ptt_tx_lease is None  # noqa: SLF001
        assert session.tx_demand == 1  # only the foreign lease remains...
        assert not foreign.released  # ...and it was NOT released
        assert radio.state == "idle"
    finally:
