...
Every step of that sequence is delivered, not merely recorded. Writing ACK, RELEASE or ACCEPT into the handoff document wakes nobody, so the party that records a step also sends it to the counterpart in the same turn as a direct session message, saying where the line was written and what it verified. Address that counterpart by the worktree and branch the checkpoint names, resolved against the host's session list, rather than by a session title, which need not match the identifier the checkpoint uses. Self-identify in the message so that it stays meaningful if it reaches the wrong session. A successor that has ACKed does not then wait silently for RELEASE: if the counterpart does not answer, report the stall to the owner and name what it blocks. Waiting is not a state of this protocol, and a step nobody was told about has not been taken.

Record and send the identifier the host's session list reports, not an internal id taken from a tool path or a scratchpad directory: the message tool refuses the latter with `No agent named … is reachable`, so an address that does not resolve leaves the counterpart unable to answer and forces the party being released to repair the handover instead of ending it.

Confirm every write to the handoff document by re-reading the document. The mutation's own response is not readback: Linear's `documentUpdate` answers `success: true` while echoing the `updatedAt` the document carried before the write. Check something that can fail — the byte count against what was sent, the heading count, the whole last sentence written, and the presence of the oldest section as well as the newest, because a write that truncates the history still leaves the newest heading exactly where its author looks for it.
...
Creating a PR, a failed attempt, or a context rollover does not authorize destructive cleanup. A completed merge does: remove the merged PR's worktree and branch immediately (owner, 2026-09-16). Check three things first, each of which leaves ownership unreleased or work unretained: an open PR based on the merged branch (deleting the base auto-closes the child irrecoverably; retarget children first, per repository rules), uncommitted changes in the worktree, and another live session working in it. Unpushed local commits are also unretained work, and are not covered by the uncommitted-changes check. Preserve active or uncertain worktrees, dirty files, workers, services, and hardware ownership. Remove a worktree only after ownership is released, required work and evidence are retained, its state is verified safe, and cleanup is within existing authorization. Do not automatically force removal or prune on startup. Never assume process termination from a client timeout; verify the remote job state before releasing its resource ownership.
