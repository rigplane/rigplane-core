...
|---|---|---|---|
| `runtime/_civ_rx.py::_update_state_cache_from_frame` | `migrated` plus `executor_cache_keep` | Supported CI-V frames call `_apply_state_store_observations(...)` before legacy mirrors. Covered by `tests/test_civ_rx_coverage.py` and state pipeline diagnostics. | Keep cache reads private to executors; do not expose `_state_cache` as consumer delivery truth. |
| `runtime/_civ_rx.py::_RADIO_STATE_HANDLERS` | `migrated` plus `compatibility_only` | MOR-437 made the in-scope Icom field families observation-backed: `_observations_from_frame(...)` emits `StateStore` observations for freq/mode, att (0x11), filter_width/agc_time_constant/data_mode (0x1A 03/04/06), power/ptt/tuner/tx-monitor (0x18/0x1C), active/dual-watch/split (0x07/0x0F), RIT (0x21), the 0x14 levels in `_CMD14_OBSERVATION_BACKED_SUBS`, the 0x15 meters, and the 0x16 toggles/values in `_CMD16_OBSERVATION_BACKED_SUBS`/`_CMD16_OBSERVATION_BACKED_VALUE_SUBS`. | `deferred_follow_up`: handlers still mirror into `RadioState` for field families with no observation emitter yet. Delete those mirrors only after each gains a documented snapshot projection and compatibility tests. |
...
| `backends/yaesu_cat/poller.py` | `migrated` plus `compatibility_only` | Web startup uses Yaesu observation poller ingress for the fields that the FTX-1 `[state_acquisition]` profile declares and that `backends/yaesu_cat/observations.py` emits: freq/mode, PTT, RX and TX meters, AF/RF/squelch, RF front-end, DSP controls, split/active receiver, RIT, tuner, dial lock, CW, and tone/tSQL fields. Legacy `RadioState` callbacks remain for compatibility poller callers only. | MOR-424 split the former broad Yaesu gap list into explicit implementation tasks and release-claim guardrails. Completed follow-ups include MOR-443 through MOR-449 plus MOR-452, MOR-458, and MOR-460 through MOR-463; remaining follow-ups such as MOR-474 and MOR-465 are tracked in `docs/internals/backend-neutral-readiness-gap-register.md`. `monitor` is correctly **excluded** (FTX-1 has no CAT monitor — ML is rejected, EX-menu only), not a gap. No current release claims FTX-1 backend-neutral control parity without the MOR-348 hardware checklist or an explicit waiver. |
