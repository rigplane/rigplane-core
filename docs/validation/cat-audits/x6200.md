...
**Manual:** Radioddity **"XIEGU X6200 CI-V implementation V1.0.6"** (Firmware V1.0.6, dated 2025-06-20, 13 pp.) — official, tabular, **CONFIRMED** for its documented subset. Cross-referenced against Hamlib `rigs/icom/xiegu.c` (`x6100_priv_caps`, reused by X6200; `RIG_MODEL_X6200 = 3091`). Sources + verbatim extract: `manuals/x6200.txt` (+ `manuals/x6200_civ_v1.0.6.pdf`) — kept locally under the git-ignored `manuals/` per `docs/validation/cat-audits/README.md` § Source manuals; not redistributed in this public repo (correction, D2 MOR-2019: this line previously said "committed", which was false — the earlier fix that dropped the pointer entirely, reasoning from `manuals/`'s absence from git history, was itself wrong: a git-ignored path is invisible to history by construction, so "absent from history" does not mean "never existed" — the files are present on disk, just never committed, as intended). **Confidence: high** for documented opcodes; Hamlib `X108G_LEVELS/FUNCS` over-advertise — the Radioddity table wins.
...
| `0x03` freq get (legacy) | ⚠️ narrative only | `get_freq` → **main RX path uses hardcoded `0x03`** | `get_freq=[0x03]` (declared, not wired) | via `freq.*` structural · not run |
| `0x05` freq set (legacy) | ❌ (absent from table and narrative) | `set_freq` → **main RX path uses hardcoded `0x05`** | `set_freq=[0x05]` (declared, not wired) | via `freq.*` structural · not run |
...
| RIT/XIT | `rit.set`,`xit.set` write-only checks (caps `rit`,`xit`) | `0x21` RIT/XIT — **not in the X6200 table**. Already partly hedged via `[validation] write_only_controls=["rit","xit","notch"]` ("SET accepted but GET times out", MOR-179/207 — live-confirmed **on this X6200 unit itself** via commits `5a493d70` and `b1df1e71`, not another Icom model; correction, D2 MOR-2019 — this row previously claimed the opposite). The `0x21` opcode itself is still undocumented in the V1.0.6 table — the *behavior* is live-confirmed, the register isn't. | **NEW** (med) — confirm `0x21` on X6200 HW; if absent, drop `rit`/`xit` caps |
...
| legacy freq get `0x03` | `get_freq` main path (hardcoded `0x03`) | `0x03` appears only in the page-4 narrative example, not Table 1. Likely works (example is explicit) but unconfirmed; `0x25 0x00` is the table-blessed path. | **NEW** (med) — confirm; prefer `0x25 0x00` for main if `0x03` is flaky |
| legacy freq set `0x05` | `set_freq` main path (hardcoded `0x05`) | `0x05` has no documentary basis at all — absent from both Table 1 and the page-4 narrative, which shows the GET direction only. Unlike `0x03`, there is no worked example to even suggest it responds. | **NEW** (med) — confirm `0x05` responds on HW; if not, route main frequency writes through the dual-RX `0x25` path instead |
