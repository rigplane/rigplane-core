...
**Manual:** Radioddity **"XIEGU X6200 CI-V implementation V1.0.6"** (Firmware V1.0.6, dated 2025-06-20, 13 pp.) — official, tabular, **CONFIRMED** for its documented subset. Cross-referenced against Hamlib `rigs/icom/xiegu.c` (`x6100_priv_caps`, reused by X6200; `RIG_MODEL_X6200 = 3091`). **Not vendored in this repository** — no `manuals/` directory exists anywhere in this repo's git history (correction, D2 MOR-2019: this line previously claimed a committed `manuals/x6200.txt` + `manuals/x6200_civ_v1.0.6.pdf`, which never existed); the manual is cited here by title, version and date only. **Confidence: high** for documented opcodes; Hamlib `X108G_LEVELS/FUNCS` over-advertise — the Radioddity table wins.
