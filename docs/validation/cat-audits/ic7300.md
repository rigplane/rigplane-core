...
The official Icom **IC-7300 Advanced Manual (11a), §19**, remains the command
...
`a73a9600899e1e900bc5fbb0ecbec33d1883f68b5ff54905a660013f5f00c66c`, and
profile SHA-256 `355cf05958e5f3537fe8680bfec38337e44e0f725cc11506ff8c76254c26a778`.
Its read-only inputs were `ic7300-manual-profile-census.md`
(`5f5a986819b1c80ba2a4b923e296d76d4254948f56549e7540155d0568cf2384`) and JSON
(`fb6303d051b22fcbe6f78b598d60b6358c29bbc85f753b95c6c8f9a9841e4ff1`).

### Reconciled unit of count

- 371 manual semantic rows are **evidence rows**, across 323 raw command keys.
- 150 rows lack a profile representation, across 143 raw keys: 141 callable
  raw-key candidates and two inbound notifications (`00`, `01`).
- Normalization yields a conservative ceiling of **137 callable domains**, not an
  actionable API/addition count. Scope edges, repeated customization addresses, and
  menu families still require an explicit domain design.
- Seven existing generic declaration groups already represent 40 selector/value rows
  (`07`, `08`, `0E`, `0F`, `10`, `13`, `18`); they must not gain selector-named APIs.
- Approved bulk profile additions: **0**. The CSV has one stable row per collapsed
  candidate domain, notification, verified mismatch, or generic-coverage group; its
  `source_record_ids` trace back to the supplied JSON.

### High-confidence semantic mismatches

These are individual audit findings, separate from the 137-domain ceiling:

| Official bytes | Current declaration | Manual meaning / next boundary |
|---|---|---|
| `1C 03` | `get_/set_tx_freq_monitor` | Read transmit frequency, not a boolean monitor; the setter is undocumented. A replacement needs a frequency response/parser design. |
| `1A 05 00 40` | `get_/set_scope_marker_position` | Speech speed; marker replacement must decide menu (`1A 05 01 00`) versus live scope (`27 20`) semantics. |
| `1A 05 00 70` | `get_/set_ref_adjust` | RTTY-memory external-keypad setting; manual reference frequency is `1A 05 00 58` with `0000..0255`, not the current domain. |

The manual evidence above is distinct from hardware evidence. Apart from the earlier
read-only APF observation recorded in this audit, this census makes no new bench,
RX, TX, or validation-matrix claim.
