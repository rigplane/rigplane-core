...
Twenty-three rows. "Evidence" is a test name, a file:line, or a verify report; "disposition" is what
...
| **P13** | **Unknown announced as not-pressed (MOR-1358).** Six controls emit a boolean `aria-pressed` — `FilterSurface.svelte:114,129,160`, `BandSurface.svelte:187`, `DspSurface.svelte:165,177` — so an unobserved fact is announced as "not pressed". Nine more do the same with `aria-checked` on `role="radio"`: `RfFrontEndSurface.svelte:130,151` (PREAMP/ATT), `CwKeyerSurface.svelte:171,229`, `ScopeControlsSurface.svelte:120,133`, `RxAudioSurface.svelte:124,157,174`, `AntennaSurface.svelte:169`. **RIT/XIT and the RfFrontEnd `aria-pressed` toggles are already fixed** and emit `undefined` | `recorded lack` | file:line above. The affected six are *pinned as intended*: `FilterSurface.test.ts:188,415` and `BandSurface.test.ts:334` all assert `'false'` — fixing MOR-1358 requires editing those assertions | **MOR-1358.** Scope corrected in §3.5; the correct pattern already exists in-repo (`VfoSurface.svelte:245-247`, `triState` → `'mixed'`) |
...
| **P20** | **Spectrum-toolbar boundary.** Twelve fact-backed leaves retire under `hideScopeControls` (MODE, EDGE 1-4, SPAN, SPEED, HOLD, REF, DUAL + MAIN/SUB, the mobile REF row, and the settings-gear popover). Nine client-side view options render unconditionally: STEP, `VIEW ON/OFF`, AVG, PEAK, BRT (×2 rows), colour scheme, BANDS, layers/region, EiBi, fullscreen | `by design` | `SpectrumToolbar.svelte:40-73` states the rule; retiring set at `:213-284,323-330,393-401`, surviving set at `:180-212,293-347,348-406`; `SpectrumToolbar.component.test.ts` pins both directions | none. **Correction:** the ruling names eight survivors; there are **nine** — `VIEW ON/OFF` (`:205-212`) is not in the S10/S6b list |
...
### 3.5 MOR-1358 is two-thirds fixed and twice as wide (P13)

RIT/XIT (`RitXitScanSurface.svelte:64-65`) and the RfFrontEnd `aria-pressed` toggles
(`:183`) already emit `undefined` for unknown. But the same defect class exists on nine
`aria-checked` `role="radio"` controls the seed never named, including PREAMP and ATT — which is
where the seed thought the `aria-pressed` bug was. The affected six `aria-pressed` sites are
*pinned as intended behaviour* by three tests that assert `'false'`, so MOR-1358 is not a one-line
fix.
...
call sites — every `core.vfo.*` key, all 18 of which are translated in `ru-RU` and `ja-JP`. The other
thirteen surfaces hardcode English.
...
6. **Harness widening** — mounting `RadioLayout` under `desktopV2Layout` in `fixtures/main.ts` is the
