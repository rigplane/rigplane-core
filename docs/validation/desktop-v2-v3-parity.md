...
Twenty-three rows plus four sub-rows (P1r, P13c, P13r — the last two added by `verify-mor-1372`
when the MOR-1358 row was split; see §3.5 — and P10r, added by this same landing per
`verify-mor-1356` §7 N2). "Evidence" is a test name, a file:line, or a verify
...
|---|---|---|---|---|
| **P1** | **Meters peak-hold (MOR-1282).** The seed recorded this as a lack. It is **not** one at `7612a4f8`: `MetersSurface` enables peak-hold per meter (`MetersSurface.svelte:57-64`, `showPeak` true for Po/SWR/ALC/Id, mirroring the dock's own `PeakKey` set) and delegates the ballistics to `BarGauge`, which holds the state (`BarGauge.svelte:63,76`), decays over the shared `PEAK_DECAY_MS = 1500` (`meter-utils.ts:361`) and resets on `ondblclick` (`BarGauge.svelte:120`) | `parity` | `MetersSurface.isolated.test.ts: BarGauge peak channel (MOR-1282)`; `MetersSurface and MetersDockPanel are on the same peak-hold channel`; `BarGauge.peakhold.svelte.test.ts:57` | **closed.** See §3.1 for the correction, and the residual below |
| **P1r** | **Peak-marker relevance gate (residual of P1, new here).** The dock hides the marker on an irrelevant tile (`MetersDockPanel.svelte:422`, `&& tile.relevant`); `BarGauge` has no relevance gate, so an RX-dimmed Po tile shows a peak marker on the semantic surface and none in the dock | `recorded lack` (cosmetic) | `MetersDockPanel.svelte:422` vs `MetersSurface.svelte:145-148,168`; no test either side | **MOR-1385** — one-expression fix in the `{@const}` block MOR-1345 introduced (`MetersSurface.svelte:161`): `showPeak={showPeak && isObserved && meters[field].relevant}` (`verify-mor-1345` §7). Scope it as a surface-level change, not a `BarGauge` prop change — the relevance decision belongs to the surface, mirroring where MOR-1345 put the fault decision. |
| **P2** | **SWR/ALC fault border (MOR-1345).** Closed. The dock draws a fault border for SWR > 2.0 and ALC ≥ redline during TX; the semantic surface now reproduces it via the same shared predicates. Note the seed's framing is wrong: `fault` is not a tile-component prop, it is a field on `MetersDockPanel`'s own local `Tile` type — `MeterPanel` and `DockMeterPanel` never had it | `parity` | legacy: `MetersDockPanel.svelte:75,272,286,400,495` + `meter-utils.ts:327,332`, pinned by `MetersDockPanel.isolated.test.ts:433`. Semantic: `MetersSurface.svelte:75-82,161-169` computes `fault` from the same `isSwrFault`/`isAlcFault` predicates and threads it into `BarGauge`, pinned by `MetersSurface.isolated.test.ts: SWR/ALC fault highlighting reuses the dock's own threshold` + `BarGauge.fault.test.ts`; `verify-mor-1345` §§2-6 | **closed by MOR-1345 at PR #2280.** |
| **P3** | **Whole-group-absent meters (S5-N1).** A radio reporting no meter fields renders **nothing** on the semantic surface (`MetersSurface.svelte:112`, `{#if meters}` around the whole section). The seed said the legacy dock showed `?` placeholders. It did not — it rendered a titled empty shell (`MetersDockPanel.svelte:385-392` are unconditional; tiles are push-gated at `:243-323`). The `?` is the **semantic** surface's per-field behaviour (`MetersSurface.svelte:132,150`) | `changed derivation` | `MetersSurface.isolated.test.ts: renders NOTHING at all when the view model carries no meters group`; `semantic-meters-wiring.component.test.ts:244` | **accepted, direction corrected** — §3.3 |
| **P4** | **SUB tuning on dual-receiver radios (MOR-1335).** Closed. `isActive` is a single radio-wide fact, so at most one VFO tile per **radio** was tunable and, with `state.active === 'MAIN'`, both SUB tiles were read-only where the legacy `VfoPanel` mounted one tunable widget per **receiver** (`DualVfoDisplay.svelte:54,68` → `VfoPanel.svelte:78`). MOR-1335 adds `VfoViewModel.isActiveSlot` — "this VFO is the active slot OF ITS OWN RECEIVER" (`radio-view-model.ts:91`, produced at `radio-view-model-adapter.ts:1278`) — and re-keys the gate to it (`VfoSurface.svelte:232`, handler re-guard `:236`, observable as `data-vfo-active-slot` at `:334`). The decomposition is exact (`isActive === isActiveSlot && this receiver is the active receiver`), verified over a 3304-cell differential matrix against the pre-change adapter: 0 differences on every pre-existing fact, 0 identity breaks. Two vocabulary invariants make the gate's safety checkable rather than conventional: at most ONE active slot per receiver, and `isActive` ⇒ `isActiveSlot` (`radio-view-model.ts:1601-1626`). **Two residual differences from legacy, both deliberate:** v3 is STRICTER when a receiver's `activeSlot` was never observed (legacy mounted the widget unconditionally; v3 fails closed on both slots, never guessing 'A'), and v3 mounts on the receiver's active-SLOT tile — same physical VFO, read from `<rx>.vfoA\|vfoB.freqHz` rather than `<rx>.freqHz` | `parity` (+ `by design`, fail-closed) | `VfoSurface.test.ts:1144` *"2/main_sub mounts exactly one control per RECEIVER, MAIN and SUB"*; `:1152` *"each control dispatches its OWN receiver and its OWN tile frequency"*; `:1207` *"an unobserved active slot leaves that receiver untunable while the other still tunes"*; `radio-view-model-adapter.test.ts:203,226`; `radio-view-model.test.ts:217,225`; end-to-end through the real adapter and wiring at `semantic-tx-aux-wiring.component.test.ts:354` (the default-path outline now carries TWO digit controls where it carried one); `verify-mor-1335` §2, §5, §6 | **closed by MOR-1335 at `PR #2283`.** **Correction to this row's earlier disposition:** it said *"the fix must add a receiver axis, not remove the gate"*. The gate was never missing a receiver axis — `isActive` was already the conjunction (active RECEIVER) ∧ (that receiver's active SLOT). MOR-1335 split that conjunction and gated on the **slot** term alone, deliberately dropping the receiver-identity term. That is correct, not a widening of the hazard: B1 is an INTRA-receiver hazard (which VFO of this receiver `set_freq {receiver}` lands on — `radio_poller.py:1439-1466`), and the receiver-identity term was never load-bearing because the intent carries `vfo.receiver` and the wiring routes MAIN→`receiver: 0` / SUB→`receiver: 1` (`SemanticRadioSurfaces.svelte:416-419` → `command-bus.ts:132-139`), so a SUB tile has no reachable path to MAIN |
...
| **P6** | **Audio-link-lost readout (new in this document).** Closed. `RxAudioPanel.svelte:53-54` rendered an output indicator that read `core.overlay.audioLinkLost` when live monitoring was selected and the browser audio transport was down, and its sole consumer retired with the panel. MOR-1384 restores the indication on `RxAudioSurface` from the **same underlying fact**, not a new one: `rxAudio.liveAudio.operational` is `runtime.connectionAudio` handed in as `RxAudioSnapshot.connected` (`SemanticRadioSurfaces.svelte:335` → `radio-view-model-adapter.ts:1195`) — the identical audio-WS health the panel read through `deriveRxAudioProps`'s `isAudioConnected`. It renders as a bare, zero-focusable `<p data-testid="rx-audio-link">live audio link lost</p>` beside the monitor row (`RxAudioSurface.svelte:73,122-124,152-158`), so the `rx-audio` zone's tab order is unchanged (MOR-1069/MOR-1304 mounting canon) and the readout is operator-visible on the default `desktop-v2` workspace, which declares an `rx-audio` zone (`desktop-declarations.ts:108`). **Gate:** `liveAudio.structural ∧ monitorMode === 'live' ∧ liveAudio.operational === false`. The `live` leg is the **epistemic** one — the audio WS opens *because* the operator selects `live`, so a link that was never requested (`local`/`mute`) is never narrated as lost — and the v2 string's `— reconnecting…` clause is deliberately dropped: a retry in flight is client behaviour this contract does not carry and the surface may not consult the audio manager to learn it. English hardcoded, per the neighbours (MOR-1373 policy still open) | `parity` (restored) | `RxAudioSurface.test.ts:197` *"renders the link-lost readout while live is selected and the link is down"*; `:206` *"states the loss as TEXT inside the surface, not only as an attribute"*; `:214` *"renders nothing at all while the engaged link is up"*; `:226` *"makes no loss claim while local/mute is selected, even with the link down"* (the epistemic pin); `:237` *"makes no loss claim on a radio whose live audio is structurally absent"*; `:247` *"adds no focusable element to the surface"*. Three mutants killed (drop the `live` leg → `:226` fails; drop the structural leg → `:237` fails; invert the observed-loss check → four fail) — `build-mor-1384` §4 | **closed by MOR-1384.** The doctrine question the row raised is answered narrowly: transport health is a client fact, and it is admitted here only as the annotation the `liveAudio` fact **already** carries (`RxAudioSurface.svelte:123` renders it as `data-live-link` on the `live` choice since MOR-1279) — this slice gives that same fact words. No new fact, no new prop, no new store read |
...
| **P10** | **`deriveBand` reads `state.active` with no freshness gate (MOR-1307 N4).** Closed. `deriveBand`'s TX permit used to read `state?.active === 'SUB'` raw while the sibling `activeReceiver` derivation required the three-part `seen()` gate (observed **and** fresh **and** available). With `active` unobserved or stale, `onSub` silently defaulted to **MAIN** and the adapter published `currentBandTx: 'allowed'` while simultaneously emitting `{ field: 'activeReceiver', code: 'field-not-observed' }`. MOR-1356 extracts `activeReceiverId()` (`radio-view-model-adapter.ts:95`) as the SAME criterion both `activeReceiver` and `deriveBand` now share, and gates the permit on `activeConfirmed = activeReceiverId(state) !== null` (`:729`, consumed at `:753`) — the permit can no longer rest on an unconfirmed receiver | `by design` (fail-closed) | `radio-view-model-adapter.ts:95,729,753`; 25-cell differential matrix confirms the extraction did not move `activeReceiver` (`verify-mor-1356` §2); `band-adapter.test.ts:366` *"currentBandTx fails closed on an unconfirmed active receiver (MOR-1356)"* — 5 denial cases incl. "no status entry" `:374`; M1/M2/M4 mutation kills (`verify-mor-1356` §3) | **closed by MOR-1356 at PR #2279.** Residual moved to **P10r** below — the denial the fix makes newly reachable is reported with a misattributed reason |
| **P10r** | **Band-TX denial on an unconfirmed active receiver is reported with a misattributed reason (residual of P10, new here).** The fix converts a fail-open permit into a fail-closed denial, but the surface tells the operator "current band could not be resolved" while displaying the resolved band directly above it — `txDeniedReason` filters on `field === 'txPermit'` (`BandSurface.svelte:102-108`) and drops the real `{ field: 'activeReceiver', code: 'field-not-observed' }` reason, so the `UNRESOLVED_REASON` fallback fires. Band-choice buttons ARE disabled in this state, giving a non-verbal signal, but no words-level account is correct | `by design` (fail-closed) + **epistemic gap** | `BandSurface.svelte:102-108`; three-way rendering table + rendered-output reproduction at `verify-mor-1356` §4.2 | **needs a ticket** — add an honest reason (a new `DisabledReasonCode` surfaced through `TX_REASON_CODES`/`REASON_LABEL`, or an adapter-side `field: 'txPermit'` reason) and pin it with a rendered-output test (`verify-mor-1356` §4.4) |
...
| **P13** | **Unknown announced as not-pressed (MOR-1358) — TOGGLES ONLY, and they are done.** The defect is a two-state toggle emitting `aria-pressed="false"` over an unread reading, which claims "this control is OFF" about a fact the radio never reported. Every such toggle in the family now emits `undefined`: `TxAuxSurface:113`, `DspSurface:124`, `RitXitScanSurface:138,144,167`, `RfFrontEndSurface:183`, `CwKeyerSurface:208,247`, `ScopeControlsSurface:176`, `AntennaSurface:181` (`valueOf(ant.rxAnt)` — the same shape under a different name, missed by every `pressedOf` grep) | `parity` — closed | shared helper + unit pin at `semantic/pressed-of.ts` / `__tests__/pressed-of.test.ts`; DOM omission pins at `DspSurface.test.ts`, `RitXitScanSurface.test.ts` ×3, `CwKeyerSurface.test.ts` ×2, `AntennaSurface.test.ts:221`; mutation-killed 8× (verify-MOR-1358 §4) | **closed.** MOR-1358 landed as #2277 (`e57c4ee7`); MOR-1383 completed the migration on both surfaces the row named — `ScopeControlsSurface` at `e875de75` and `AntennaSurface:182` at `c7254199` (the `valueOf` alias that hid it from every `pressedOf` grep is gone from the aria binding; `valueOf` survives only for the `txAntenna` VALUE read at `:140`, which `pressedOf`'s `boolean \| AtuStatus` parameter structurally cannot take). All seven consumers now derive `aria-pressed` from one pinned helper. **P13r** — the unpinned filter-selection highlight — is NOT covered by either ticket and still needs its one-line test pin |
...
semantic surface and the legacy dock now sit on, and `MetersSurface.isolated.test.ts: MetersSurface and MetersDockPanel are on the same peak-hold channel` asserts exactly
...
| `core.overlay.audioLinkLost` | `Audio link lost — reconnecting…` | `RxAudioPanel.svelte:54` | `RxAudioSurface.svelte:73` renders `live audio link lost` — hardcoded, and without the `— reconnecting…` clause (MOR-1384, see P6). So the *indication* is restored and the *key* still loses its only rendering surface: the `ru-RU`/`ja-JP` loss stands, and this row remains MOR-1373's |
...
translated home (all three now have an English one: `audioLinkLost`'s was restored by MOR-1384),
and (b) a policy question: thirteen semantic
...
1. **Two unticketed deferrals must be filed or explicitly confirmed as deferred before MOR-1263
   ...
   not a disposition.
2. ~~**P6 (audio-link-lost) has no ticket.** It was found by this slice.~~ **Resolved** — filed as
   MOR-1384 and closed by it; see the P6 row.
...
6. ~~**MOR-1358 must land before this row closes.** The fix is built and independently verified at
   `0a1ecf60` but is not on `main` (P13).~~ **Landed** as #2277. ~~**MOR-1383 must be widened to two
   surfaces** (`ScopeControlsSurface` *and* `AntennaSurface`, which carries the same shape under the
   name `valueOf`).~~ **Done** at `c7254199` (`verify-mor-1383` appendix). **P13r** — the unpinned
   filter-selection highlight — still needs a one-line test pin.
