...
MOR-1905 (`c87c59c3`), MOR-1940 (`9fc90943`, FREQUENCY/RIT_XIT reclassified
TX_SAFE — §2 P1), MOR-1909 (`67fbcbea`, row 1), MOR-1910 (`94168f4e`, row 2),
MOR-1912 (`ad89d10c`, row 3a), MOR-1911 (`9a05ecb0`, row 3b), MOR-1913
(`07d40580`, row 4), MOR-1914 (`24eac81d`, row 5), MOR-1941 (`a1cf9f48`,
row 6), MOR-1947 (`33947560`, `[tx_policy]` data for the six unmeasured rigs),
MOR-1954 (`dbeb4511`, the T5 argument-predicate short-circuit). Citations
elsewhere in this document are still anchored at `769bfc71` unless the
...
(`core/tx_authority.py:486-493`), so the in-body form is a `with` block, not a
...
needs the yielded `TxAdmission` (`core/tx_authority.py:394-404`) in scope.
...
MAIN/SUB select through `_set_vfo_wire` — and the owner ruled them into the
hazard family on 2026-08-21, closing Q13. See §3.3.)
...
body. Row 7's map must therefore enumerate public aliases as a class of its
...
| `DeferredTxCommandLane` + all **three** construction sites (`radio_poller.py:723`, `yaesu_cat/poller.py:138`, and `yaesu_cat/poller.py:788` inside `_cancel_deferred_entry`, which rebuilds a fresh lane on cancel) + TTL/quiet constants. Corrected from "both instances", 2026-08-21: there are two lanes but three constructions, and a row-11 PR that deletes the two named sites leaves a live `DeferredTxCommandLane()` for row 15b's module deletion to break | **Deleted** (rows 9, 11, 15b) — nothing holds writes anymore |
...
Q14 and the newly raised Q15 are open and marked as such in the table below. The record is
...
| Q13 | Class of the two unclassified vfo-family members — `swap_main_sub` (`runtime/_dual_rx_runtime.py:309-326`) and `select_receiver` (`:447-471`) | **CLOSED — ruled 2026-08-21: both join the vfo-select HAZARD family.** They are now in the §3.3 table and in row 7's admission list. The wire evidence behind the ruling: `swap_main_sub` sends the same `_CMD_VFO` frame as its already-ruled twin `equalize_main_sub`, and `select_receiver`'s MAIN/SUB select is indistinguishable on the wire from `set_vfo_slot`. **Still open under the same heading**, and deliberately not classified: `set_bsr`, `set_tx_source`, `set_cross_band_split`, `vfo_a_to_b`, `vfo_b_to_a` (§3.3) | **Owner** |
| Q15 | Does the retained `connection_epoch_bootstrap` exemption reach the backend admission? | **OPEN — raised 2026-08-21, not decided.** It does not, and this document does not resolve it: the flag is a keyword parameter of `web/radio_poller.py::_execute`, `TransmitAuthority.admit` has no channel for it, and the exempted `SelectVfo(vfo="A")` dispatches into `_set_vfo_slot_impl` — the seat §3.2 designates. §3.5 states both halves; rows 7-9 must not be read as having settled it | **Owner** — pending |
