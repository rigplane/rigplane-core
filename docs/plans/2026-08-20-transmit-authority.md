...
2026-08-21 and Q15 ruled 2026-08-21 by MOR-1959, **Q14 open**; the MOR-1792
baseline obligation stands (§10 — PR #2745 has since merged with no
on-record discharge of it)
...
**The connect-time bootstrap exemption is deleted. RESOLVED (Q15, ruled
2026-08-21 by MOR-1959).** The `connection_epoch_bootstrap` parameter this
section originally described no longer exists. The owner ruled that the
connection-epoch bootstrap `SelectVfo` takes the same admission as any other
write — no exemption channel, no bypass flag, no forked wire write. When
that admission refuses it — the structural case at connection start, since
RF/VFO truth is unobservable until that first `SelectVfo` lands — the
poller re-attempts the write with backoff
(`web/radio_poller.py::establish_vfo_identity`) rather than abandoning it.

This section previously argued the point in two halves, because an earlier
revision asserted only that the exemption survives its own row and a later
one added that it cannot travel to the backend admission rows 7–8 create.
MOR-1959 removes the premise under both: there is no exemption left to
survive a row or fail to travel anywhere. `_execute` carries no
`connection_epoch_bootstrap` parameter or skip branch — confirmed by
`tests/test_radio_poller_tx_interlock.py::test_connection_epoch_bootstrap_parameter_is_fully_removed`
— so a future backend admission (rows 7–8, unbuilt) has nothing special to
wire up for this write at all; it is ordinary from the day it lands.
...
| The `connection_epoch_bootstrap` exemption | **Already deleted — MOR-1959, ruled 2026-08-21, ahead of rows 9–11.** The bootstrap `SelectVfo` now takes the same admission as every other write; a refusal is retried with backoff in `web/radio_poller.py::establish_vfo_identity` instead of being exempted. Resolves Q15: there is no longer a flag for a future backend admission to fail to carry |
...
questions raised on 2026-08-21, Q13 and Q15 were ruled the same day and are
closed here; Q14 remains open, marked as such in the table below.
...
| Q15 | Does the retained `connection_epoch_bootstrap` exemption reach the backend admission? | **RESOLVED — ruled 2026-08-21 (MOR-1959); postdates the §9 record and is not in it.** The exemption is deleted, not retained: the bootstrap `SelectVfo` takes the same admission as any other write, and a refusal — the routine case at connection start — is retried with backoff in the poller's own run loop (`web/radio_poller.py::establish_vfo_identity`) rather than exempted or abandoned. The original question is moot: there is no separate flag left for any admission layer, present or future, to fail to carry | **Owner** — MOR-1959 |
...
settled, and Q13 and Q15 were ruled on 2026-08-21 — Q15 by MOR-1959, which
deleted the connect-time bootstrap exemption this document had described as
retained (§3.5) — but **one remains open: Q14** (does the Icom path parse
through `tx_state_map`). This sentence claimed all of them were settled until
2026-08-21; it was already wrong about Q14, and Q15's own resolution must be
visible from wherever a reader arrives (§9); the bench
