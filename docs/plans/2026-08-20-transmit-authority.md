...
`web/radio_poller.py:3010`, `rigctld/handler.py:2208`,
`runtime/profiles_runtime.py:63,70` and `set_cross_band_split`. Each body needs
...
`select_receiver`) and 33 reads (`read_* → get_*`), the reads mattering only
for keeping INV-1's non-write allow-list honest.
