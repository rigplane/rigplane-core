...
Canonical location: icom_lan.core.exceptions
...
makes ``icom_lan.exceptions`` literally the same module object as
``icom_lan.core.exceptions``. This preserves attribute walks (incl.
...
* ``from icom_lan.core.exceptions import *`` — static-analysis adapter.
  Mypy and ruff resolve re-exported names through star-imports; they
  do not model the ``sys.modules`` mutation. Without this line,
  every consumer of ``from icom_lan.exceptions import X`` triggers
...
* ``sys.modules[__name__] = _canonical`` — the runtime invariant.
  Makes ``icom_lan.exceptions`` and ``icom_lan.core.exceptions`` the
...
from icom_lan.core.exceptions import *  # noqa: F401, F403
import icom_lan.core.exceptions as _canonical
