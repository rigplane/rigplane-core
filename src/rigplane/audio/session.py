...
- ``"tx_first"`` / ``"atomic"`` (same-device exclusive USB): the TX leg
  ...
  duplex topology INTO ``UsbAudioDriver`` itself: its ``start_tx`` /
  ``stop_tx`` / ``start_rx`` / ``stop_rx`` own the RX ↔ duplex handoff
  (ADR §3.3 item 2), so every USB backend gets it through the plain
  pass-through calls; the session's sequencing here is that seam's
  caller.
