...
Canonical location: icom_lan.core._optional_deps
...
makes ``icom_lan._optional_deps`` literally the same module object as
``icom_lan.core._optional_deps``. This preserves attribute walks (incl.
...
* ``from icom_lan.core._optional_deps import *`` — static-analysis adapter.
  Mypy and ruff resolve re-exported names through star-imports; they
  do not model the ``sys.modules`` mutation. Without this line,
  every consumer of ``from icom_lan._optional_deps import X`` triggers
...
* ``sys.modules[__name__] = _canonical`` — the runtime invariant.
  Makes ``icom_lan._optional_deps`` and ``icom_lan.core._optional_deps`` the
...
from icom_lan.core._optional_deps import *  # noqa: F401, F403
import icom_lan.core._optional_deps as _canonical
