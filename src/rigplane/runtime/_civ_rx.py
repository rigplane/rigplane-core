...
class CivRuntime:
    ...
    async def request_authoritative_ptt_read(
        ...
        *,
        ...
        The cost is that a refusal genuinely aimed at *this* read no longer
        ends it: the read runs out ``_civ_get_timeout`` and fails as a timeout
        (``tests/test_raw_civ_transaction.py:
        test_authoritative_ptt_read_waits_out_its_own_refusal``). Both are read
        failures downstream — ``managed_tx_effect_service.py`` settles the
        attempt on the task's exception either way — so this trades a
        bounded delay for never charging one command's refusal to another.
        """
    ...
    async def execute_civ_transaction(
        ...
        *,
        ...
        meanwhile falls through to the tracker's orphan backlog. The cost is
        that a refusal genuinely aimed at *this* request no longer ends it —
        it runs out ``timeout`` and fails as a timeout instead.
        """
