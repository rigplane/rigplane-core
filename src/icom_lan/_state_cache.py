...
Canonical location: icom_lan.core._state_cache
...
makes ``icom_lan._state_cache`` literally the same module object as
``icom_lan.core._state_cache``. This preserves attribute walks
(incl. stdlib names not in ``__all__``) and monkeypatch targets such
as ``unittest.mock.patch('icom_lan._state_cache.…', …)``.

The two import lines below are BOTH load-bearing — do not remove
either:

* ``from icom_lan.core._state_cache import *`` — static-analysis
  ...
  ``from icom_lan._state_cache import X`` triggers ``attr-defined``
...
* ``sys.modules[__name__] = _canonical`` — the runtime invariant.
  Makes ``icom_lan._state_cache`` and
  ``icom_lan.core._state_cache`` the same module object so
...
from icom_lan.core._state_cache import *  # noqa: F401, F403
import icom_lan.core._state_cache as _canonical
