...
Canonical location: icom_lan.core._queue_pressure
...
makes ``icom_lan._queue_pressure`` literally the same module object
as ``icom_lan.core._queue_pressure``. This preserves attribute walks
(incl. stdlib names not in ``__all__``) and monkeypatch targets such
as ``unittest.mock.patch('icom_lan._queue_pressure.…', …)``.

The two import lines below are BOTH load-bearing — do not remove
either:

* ``from icom_lan.core._queue_pressure import *`` — static-analysis
  ...
  ``from icom_lan._queue_pressure import X`` triggers ``attr-defined``
...
* ``sys.modules[__name__] = _canonical`` — the runtime invariant.
  Makes ``icom_lan._queue_pressure`` and
  ``icom_lan.core._queue_pressure`` the same module object so
...
from icom_lan.core._queue_pressure import *  # noqa: F401, F403
import icom_lan.core._queue_pressure as _canonical
