...
Canonical location: icom_lan.core.auth
...
makes ``icom_lan.auth`` literally the same module object as
``icom_lan.core.auth``. This preserves attribute walks (incl.
...
* ``sys.modules[__name__] = _canonical`` — the runtime invariant.
  Makes ``icom_lan.auth`` and ``icom_lan.core.auth`` the
...
from icom_lan.core.auth import *  # noqa: F401, F403
import icom_lan.core.auth as _canonical
