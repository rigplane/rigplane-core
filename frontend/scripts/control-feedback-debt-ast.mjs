...
export function collectObjectFlow(program) {
  ...
  const member = (n, s) => { if (n?.type !== 'MemberExpression') return { root: null, key: null, container: false }; const root = direct(n.object, s), p = n.property, b = n.object?.type === 'Identifier' ? get(s, n.object.name) : null; return { root, key: n.computed ? text(p, s) : p?.name ?? null, container: !!b?.container }; };
  ...
  const index = (n, s) => { if (!n || typeof n !== 'object') return; scopes.set(n, s);
    ...
    if (n.type === 'VariableDeclaration') { n.declarations.forEach((d) => { const target = n.kind === 'var' ? fnScope(s) : s, b = d.id?.type === 'Identifier' ? get(target, d.id.name) : null, alias = direct(d.init, s); if (b && n.kind === 'const' && d.init?.type === 'ObjectExpression') { b.root = { events: [] }; if (target === rootScope) { exposed.set(d.id.name, b.root.events); rootDecls.add(d); } } else if (b && n.kind === 'const' && alias) b.root = alias; if (b) { b.container = d.init?.type === 'ObjectExpression'; b.text = n.kind === 'const' ? text(d.init, s) : null; } index(d.init, s); }); return; }
  ...
  for (const root of new Set([...exposed.keys()].map((name) => get(rootScope, name).root))) {
    const reads = []; let seen = false;
    const facts = evaluateOrderedEffects(evaluated, { isTracked: (node) => { const hit = direct(node, scopes.get(node) ?? rootScope) === root; seen ||= hit; if (hit && (defaults.has(node) || deletes.has(node))) reads.push(node); return hit; } });
    ...
    records.forEach(({ fact, node }, order) => {
      ...
      const s = scopes.get(fact.node) ?? rootScope, rs = fact.kind === 'cycle' && !seen ? new Set() : new Set([root]);
      if (fact.kind === 'cycle') { cyclic ||= rs.size > 0; for (const hit of rs) hit.events.splice(0, hit.events.length, ...hit.events.filter((event) => event.poison)); }
      if (fact.kind === 'mutation' && fact.node?.type === 'AssignmentExpression') { const m = member(fact.node.left, s), key = m.key === 'feedbackPolicy' ? 'feedback-policy' : m.key, rhs = roots(fact.node.right, s); if (cyclic) poison(rs, fact.node, order); else if (m.root === root || !m.root && !m.container && !rhs.has(root)) { if (key === null || fact.node.operator !== '=') poison([root], fact.node, order); else if (key === 'type' || key === 'feedback-policy') emit(root, key, fact.node.right, false, fact.node, order); } else poison(rs, fact.node, order); }
