...
export function collectObjectFlow(program) {
  const exposed = new Map(), scope = (parent = null) => ({ parent, bindings: new Map() }), rootScope = scope();
  ...
  const names = (s, pattern, mutable) => boundNames(pattern).forEach((n) => declare(s, n, mutable));
  const predeclare = (body, s) => (body ?? []).forEach((n) => {
    if (n.type === 'VariableDeclaration') n.declarations.forEach((d) => names(s, d.id, n.kind !== 'const'));
  ...
  const roots = (n, s, seen = new Set()) => {
    ...
    const parts = ['SequenceExpression', 'ArrayExpression'].includes(unwrapped?.type) ? unwrapped.expressions ?? unwrapped.elements
      : unwrapped?.type === 'ObjectExpression' ? unwrapped.properties.map((p) => p.value)
  ...
  const member = (n, s) => {
    ...
    return { root, key: n.computed ? text(n.property, s) : n.property?.name ?? null, safe: !!root && !nested };
  };
  const walk = (n, s) => {
    ...
    if (/Function(?:Declaration|Expression)$/.test(n.type) || n.type === 'ArrowFunctionExpression') {
      const child = scope(s); if (n.type === 'FunctionExpression' && n.id) declare(child, n.id.name); n.params?.forEach((p) => names(child, p)); walk(n.body, child); return;
    }
    ...
    if (n.type === 'VariableDeclaration') { n.declarations.forEach((d) => {
      names(s, d.id, n.kind !== 'const'); const b = d.id?.type === 'Identifier' ? get(s, d.id.name) : null, alias = direct(d.init, s);
    ...
    if (n.type === 'AssignmentExpression') { const m = member(n.left, s); if (m.root) {
      const key = m.key === 'feedbackPolicy' ? 'feedback-policy' : m.key;
      if (!m.safe || !key || n.operator !== '=') poison([m.root], n.start); else if (key === 'type' || key === 'feedback-policy') emit(m.root, key, n.right, false, n.start);
      ...
      else poison(roots(n.left, s), n.start); walk(n.right, s); return; }
    if (n.type === 'UpdateExpression' || n.type === 'UnaryExpression' && n.operator === 'delete') { poison(roots(n.argument, s), n.start); return; }
    if (n.type === 'CallExpression' || n.type === 'NewExpression') { poison(roots(n.arguments, s), n.start); if (n.callee?.type === 'MemberExpression') poison(roots(n.callee.object, s), n.start); n.arguments.forEach((x) => walk(x, s)); return; }
