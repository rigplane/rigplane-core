...
**None of this is evidence tooling has a gap** — it is a coarse net with a stated mesh size. `capture.mjs`'s own assertion layer (`tx-readout`, `fault-affordance`, and the rest) already pins exactly these classes of change on every capture, every invocation, independent of pixels. **Do not treat a green pixel job as evidence that nothing visual changed** — treat it as evidence that nothing changed ABOVE ~1024 px / ~32×32 (desktop) or ~304 px / ~17×17 (phone), with the assertion layer covering the finer mesh below that for the captures above this paragraph, including all contrast/brightness drift this layer structurally cannot see. The `gallery-*` rows have no such assertion layer — `?demo=control-buttons` is never loaded by `capture.mjs` or `capture-ptt.mjs` — so for those rows the pixel floor is the only floor.
