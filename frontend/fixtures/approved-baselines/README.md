...
| `gallery-valuecontrol-hbar` | MOR-2219 looks gallery — preserves the HBar renderer's look; carried by `components-v2/controls/value-control/HBarRenderer.svelte` (owns its own track/thumb div geometry — per the MOR-2216 inventory, this is one of the renderers flagged as needing a future display-model prop, but that's a mechanism question, not a look-preservation one for this ticket) |
...
| `gallery-professionalknob` | MOR-2219 looks gallery — preserves the ProfessionalKnob skin's look; carried by `value-control/skins/ProfessionalKnob.svelte` itself (its geometry lives inside the file — SVG radial gradients, arc track, glow filter — per the MOR-2216 inventory this is the one look that independently duplicates KnobRenderer's SVG approach rather than sharing it; not resolved by this ticket, only preserved) |
...
**None of this is evidence tooling has a gap** — it is a coarse net with a stated mesh size. `capture.mjs`'s own assertion layer (`tx-readout`, `fault-affordance`, and the rest) already pins exactly these classes of change on every capture, every invocation, independent of pixels. **Do not treat a green pixel job as evidence that nothing visual changed** — treat it as evidence that nothing changed ABOVE ~1024 px / ~32×32 (desktop) or ~304 px / ~17×17 (phone), with the assertion layer covering the finer mesh below that for the first 14 captures, including all contrast/brightness drift this layer structurally cannot see. The `gallery-*` rows have no such assertion layer — `?demo=control-buttons` is never loaded by `capture.mjs` or `capture-ptt.mjs` — so for those rows the pixel floor is the only floor.
