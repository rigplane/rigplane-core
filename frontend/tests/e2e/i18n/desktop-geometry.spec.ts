...
async function standardGeometry(page: Page) {
  ...
  const instruments = await page.locator('.standard-face .receiver-instrument').evaluateAll(elements =>
    elements.map(instrument => {
      ...
      const primary = instrument.querySelector('[data-vfo-row="main"] [data-vfo-freq]');
  ...
  test('MOR-2458 keeps RX/TX geometry fixed and attributes TX only to the known split target', async ({ browser }, info) => {
    const capture = async (txState: 'rx' | 'tx', txTargetSlot: 'B' | 'unknown') => {
      ...
      const result = {
        ...
        rfFrontEndNbWidth: await page.locator(
          '[data-panel-id="semantic-rf-front-end"] [data-testid="dsp-nbWidth"]',
        ).count(),
        browserErrors,
    ...
    for (const result of [rx, tx, unknown]) {
      ...
      expect(result.rfFrontEndNbWidth).toBe(0);
  ...
  for (const width of [900, 1440]) for (const known of [true, false]) {
    test(`${layout} ${width} ${known ? 'observed IC' : 'unknown'} geometry`, async ({ page }, info) => {
      ...
      if (layout.startsWith('lcd')) {
        ...
        // Mirrors `.desktop-controls-center { min-height: 320px }` in frontend/src/components-v2/layout/RadioLayout.svelte.
        const STANDARD_CENTER_COLUMN_FLOOR_PX = 320;
        ...
        // 320px floor minus the heights of the column's other children,
        // measured on the page.
        const scopeFloor = await page.locator('.desktop-controls-center').evaluate((column, columnFloorPx) => {
          ...
          return Math.max(280, columnFloorPx - othersHeight);
        }, STANDARD_CENTER_COLUMN_FLOOR_PX);
