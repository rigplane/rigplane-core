...
export function wheelControl(node: HTMLElement, initial: WheelControl) {
  ...
  function arm() {
    ...
    // MOR-2522: the armed marker is illumination keyed on this attribute
    // (value-control.css), never an inline outline frame — the 2 px cyan
    // rectangle the owner saw on every slider click came from one.
