...
<style>
  ...
  .desktop-control-face :global(.desktop-controls-right) {
    /* Positioned so absolutely positioned descendants (sr-only text, meter
       fills, control internals) anchor to the column, not to `.radio-layout`:
       on a static column their boxes bypassed this scroll box and extended
       the layout grid's scrollable overflow, scrolling the whole face. */
    position: relative;
