 ...
 *   1. no power button at all when the radio lacks `power_control`;
...
describe('StatusBar power toggle (MOR-1673)', () => {
  ...
  it('renders no power button when the radio lacks power_control', () => {
    power.powerControl = false;
    // Deliberately ON: only the missing capability may hide the button.
