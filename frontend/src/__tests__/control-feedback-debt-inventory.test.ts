  ...
  it('exempts only the semantic host\'s exact browser-local gain identity', () => {
    const source = wrap(
      `<input type="range" aria-label={\`${'${channel.toUpperCase()}'} gain in decibels\`}
        value={value ?? 0} feedback-policy="local-resource"/>`,
      `let channel='main', value=0;`,
    );
    const file = 'src/semantic/RxAudioInstrumentHost.svelte';
    expect(debt(source, file)).toEqual([]);
    expect(() => debt(source, `nested/${file}`)).toThrow(/local-resource.*not allowed/);
    expect(() => debt(source.replace('value ?? 0', 'value'), file)).toThrow(/local-resource.*not allowed/);
    expect(() => debt(source.replace('gain in decibels', 'gain'), file)).toThrow(/local-resource.*not allowed/);
  });
