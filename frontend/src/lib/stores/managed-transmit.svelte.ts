...
export function receiveManagedTransmitSnapshot(value: ManagedTransmitDocument, receivedAt = clock()): void { document = value; const tot = value.managedTransmit.status === 'available' ? value.managedTransmit.tot : null; countdown = tot?.remainingMs === null ? null : startManagedTransmitCountdown(tot.remainingMs, receivedAt); stale = false; }
