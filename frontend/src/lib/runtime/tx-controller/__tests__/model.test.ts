...
describe('TX reducer', () => {
  ...
  it('cancels pre-key without OFF and coalesces qualified release', () => {
    ...
    expect(transition(cancelled.state, { type: 'audio-ready', guard: cancelled.state.guard!, commandId: 'late-on' })).toEqual({ state: cancelled.state, effects: [] });
  ...
  it('handles external and locally-correlated authoritative ON races', () => {
    ...
    expect(transition(rebound.state, timerEvent(rebound.state, 'on-confirmation', 'on'))).toEqual({ state: rebound.state, effects: [] });
  ...
  it('correlates ON command results and errors without RF truth or retry', () => {
    ...
    expect(transition(dispatched.state, { type: 'on-sent', guard, commandId: 'on', barrier: marker(9, 2) })).toEqual({ state: dispatched.state, effects: [] });
    expect(transition(dispatched.state, { type: 'on-sent', guard: { ...guard, leaseId: 'foreign' }, commandId: 'on', barrier: marker(2) })).toEqual({ state: dispatched.state, effects: [] });
    ...
    for (const outcome of ['ack', 'response-ok'] as const) expect(transition(dispatched.state, { ...sentEvent, outcome, barrier: marker(2) })).toEqual({ state: dispatched.state, effects: [] });
  ...
  it('preserves failed OFF obligations for errors and exact deadlines', () => {
    ...
    expect(transition(released.state, { ...sentEvent, commandId: 'wrong', offCommandId: 'wrong' })).toEqual({ state: released.state, effects: [] });
    expect(transition(released.state, { ...dispatchTimer, commandId: 'wrong', offCommandId: 'wrong' })).toEqual({ state: released.state, effects: [] });
    ...
    for (const trigger of [
      ...
      expect(types(failed)).toEqual(['cancel-timers']); expect(transition(failed.state, trigger)).toEqual({ state: failed.state, effects: [] }); expect(transition(failed.state, { type: 'release', guard: failed.state.guard!, commandId: 'off-again' })).toEqual({ state: failed.state, effects: [] });
      expect(transition(failed.state, timerEvent(failed.state, 'off-confirmation', 'off'))).toEqual({ state: failed.state, effects: [] });
      const lateOnError = { type: 'command-result' as const, command: 'on' as const, outcome: 'response-error' as const, barrier: null, ...correlation(failed.state, 'on') };
      const ignored = transition(failed.state, lateOnError); expect(ignored).toEqual({ state: failed.state, effects: [] }); expect(ignored.state.fault).toBe('release-not-confirmed');
    }
    ...
    for (const trigger of [{ type: 'command-result' as const, command: 'off' as const, outcome: 'response-error' as const, barrier: null, ...uncertainBase }, { ...timerEvent(uncertainDelivered.state, 'off-confirmation', 'off-uncertain'), offCommandId: 'off-uncertain' }]) {
      const failed = transition(uncertainDelivered.state, trigger); expect(failed.state).toMatchObject({ phase: 'failed', fault: 'release-not-confirmed', txRisk: 'uncertain', radioTx: 'off', pendingOff: uncertainDelivered.state.pendingOff, mayOwnKey: true, modRestorePending: true }); expect(types(failed)).toEqual(['cancel-timers']); expect(transition(failed.state, { type: 'release', guard: failed.state.guard!, commandId: 'off-again' })).toEqual({ state: failed.state, effects: [] });
    }
  });
  ...
  it('makes stale audio, release, authority, target and permit inputs exact no-ops', () => {
    ...
    const stale: TxEvent[] = [
      ...
      { type: 'authority', epoch: 0, ptt: ptt(false, 3), eligibility: eligible, offCommandId: 'off' },
      { type: 'authority', epoch: 1, ptt: ptt(true, 9, 3), eligibility: eligible, offCommandId: 'off' },
    ];
  ...
  it('qualifies immediate OFF evidence strictly after its bound barrier', () => {
    ...
    const stillKeyed = transition(delivered.state, { type: 'authority', epoch: 1, ptt: ptt(true, 5), eligibility: eligible, offCommandId: 'off' });
    expect(types(stillKeyed)).toEqual([]);
    expect(stillKeyed.state).toMatchObject({ phase: 'releasing', txRisk: 'confirmed-on', radioTx: 'on', modRestorePending: true, mayOwnKey: true, pendingOff: delivered.state.pendingOff });
  ...
  it('binds queued OFF once and rejects pre-drain and old-epoch completion', () => {
    ...
    const delivery = { type: 'off-sent' as const, ...off, eventEpoch: 2, barrier: marker(2, 2) };
    for (const patch of [{ commandId: 'wrong' }, { leaseId: 'wrong' }, { generation: off.generation + 1 }, { originalEpoch: off.originalEpoch + 1 }]) expect(transition(preDrain.state, { ...delivery, ...patch })).toEqual({ state: preDrain.state, effects: [] });
    const delivered = transition(preDrain.state, delivery);
