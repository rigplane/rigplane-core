 ...
 *
 * SAFETY INVARIANT R9, identical to `rfState`/`txSessionState` above: this
 * function DISPLAYS the App TX authority's conclusions; it never forms its
 * own. It takes a named struct of exactly four primitives — never a whole
 * `RendererViewModel` — so a fifth field cannot be smuggled into a TX
 * decision without changing this signature. Unrecognised values fail CLOSED
 * to the `'doubt'` rail, never to `'idle'`: "nothing is happening" is the
 * dangerous claim.
 */
...
/** True for exactly `TxSessionState`'s five members — reuses `SESSION_LABEL`'s keys rather than re-enumerating them a third time. */
const isTxSessionState = (value: string): value is TxSessionState =>
  Object.prototype.hasOwnProperty.call(SESSION_LABEL, value);

/**
 * Session → key treatment, unconditional on `keyBlocked`. Keyed by the RAW
 * session, not the resolved rail: an unrecognised session must still fall
 * through to the `keyBlocked`/doubt branch in `resolveTxFeedbackState` below
...
export function resolveTxFeedbackState(fields: {
  ...
  // Treatment follows the RESOLVED rail, not the raw session: a control that
  // still looks idle under a doubt rail is the contradiction R9 exists to
  // prevent. `keyBlocked` is checked only once the session itself has
  // nothing louder to say — see `KEY_TREATMENT`'s own comment for why.
