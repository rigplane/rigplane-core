 ...
 * this suite avoids importing wholesale, the same reason
 * `desktop-v2-registration.test.ts`'s own loader-identity pin reads source
 * as TEXT instead of importing it. Stringifying the closure obtained from
 * the live
...
const ALL_MANIFESTS = {
  ...
  'unified-instrument': unifiedInstrumentLayout,
  'panadapter-first': panadapterFirstLayout,
} as const;
...
const EXPECTED_LOADER_SPECIFIER: Readonly<Record<keyof typeof ALL_MANIFESTS, string>> = {
  ...
  'unified-instrument': '/src/skins/lcd-unified-instrument/LcdUnifiedInstrumentSkin.svelte',
  'panadapter-first': '/src/skins/lcd-panadapter-first/LcdPanadapterFirstSkin.svelte',
};
