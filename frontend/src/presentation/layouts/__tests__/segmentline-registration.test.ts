...
import { panadapterFirstLayout, peerSplitLayout, unifiedInstrumentLayout } from '../declarations';
...
describe('the additional segmentline directions are registered for production', () => {
  it.each([
    [unifiedInstrumentLayout, unifiedInstrumentGlassGroup, 540],
    [panadapterFirstLayout, panadapterFirstGlassGroup, 594],
  ] as const)('registers $id with its own fixed-native group', (layout, group, nativeH) => {
    expect(getLayout(layout.id)).toBe(layout);
    expect(layout.zones).toEqual([
      { id: 'peer-columns', surfaces: ['vfo', 'rxTx'], group: group.id },
    ]);
    expect(layout.stageSizing).toEqual({
      mode: 'fixed-native', nativeW: 1280, nativeH, minScale: 0.5,
    });
    expect(layout.fallbackLayoutId).toBe('peer-split');
  });

  it('keeps centerstage fixture-only rather than registering a production layout', () => {
    expect(listLayoutIds()).not.toContain('centerstage');
  });
});
