...
- Prose is a claim, and claims get checked. For every comment, docstring and document sentence a change adds or touches, ask: could this be false without any test failing? If so, narrow it until it is true, tie it to something that fails when it stops being true (a named constant, a named test, a parsed structure), or delete it — a guarantee stated wider than the code is worse than none, because the next reader stops checking. A claim about what a future change will do belongs in the ticket (MOR-1958). Deleting a false claim beats replacing it: a deleted sentence cannot be wrong, so if the replacement cannot be established with a command actually run, delete the claim rather than weaken it. Positional references ("above", "below", "the line after this") and counts stated in prose ("three probes", "ten zones") never belong in a comment — they rot on every edit and nothing fails when they do; drop a positional reference rather than guess one, and either derive a count where it is used or leave it unstated. Every claim carries how it was established: when a change asserts a fact, in a comment, a document, a PR body or a report, say which command or which file read established it — a corrected value that is also wrong is worse than the original it replaced. When a change fixes a false statement, review its new prose specifically — the likeliest remaining defect is in the sentences it added, so read added prose as its own pass, not only as part of the whole diff. `builder.md` and `verifier.md` point here.
