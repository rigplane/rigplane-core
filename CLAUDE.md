...
- Prose is a claim, and claims get checked. For every comment, docstring and document sentence a change adds or touches, ask: could this be false without any test failing? If so, narrow it until it is true, tie it to something that fails when it stops being true (a named constant, a named test, a parsed structure), or delete it — a guarantee stated wider than the code is worse than none, because the next reader stops checking. A claim about what a future change will do belongs in the ticket (MOR-1958). `builder.md` and `verifier.md` point here.
  ...
  - **Never state a position or a count that nothing checks.** A `file:line` in a review verdict is pinned to the SHA it names; a citation in the grandfathered baseline is checked by its own gate; a count stated beside its own enumeration — "the three workflows below", "11 layered packages" — is wrong as visibly as the list beside it. "The predicate above" and "three probes" have none of that: nothing tells you when either stops being true, and no `.github/scripts` gate or doc-sync test checks for it. Name the thing instead of its position; in code, derive a count at its use instead of writing it; in a document, state a count beside what it counts, or not at all.
